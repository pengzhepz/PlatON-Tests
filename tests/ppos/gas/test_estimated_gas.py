...
def test_estimated_gas_001(client_new_node):
    ...
    delegate_address, _ = client.economic.account.generate_account(client.node.web3, client.economic.delegate_limit + gas_fees)
    # step3：发起委托
    result = client.delegate.delegate(0, delegate_address)
    assert_code(result, 0)
...
def test_estimated_gas_002(client_new_node):
    """
       @describe: 撤销委托交易预估gas,账户余额 = 交易金额 + gas * gasPrice, 可以预估成功，且发起委托成功
       @step:
    ...
    delegate_address, _ = client.economic.account.generate_account(client.node.web3, economic.delegate_limit + gas_fees + gas_fees_withdrew)
    result = client.delegate.delegate(0, delegate_address)
    assert_code(result, 0)
...
def test_estimated_gas_003(client_new_node):
    ...
    staking_address, _ = client.economic.account.generate_account(client.node.web3, client.economic.create_staking_limit + staking_gas_fees)
    result = client.staking.create_staking(0, staking_address, staking_address, amount=client.economic.create_staking_limit, reward_per=0)
    assert_code(result, 0)
...
def test_estimated_gas_004(client_new_node):
    ...
    data = HexBytes(rlp.encode(
        [rlp.encode(int(1001)), rlp.encode(bech32_address_bytes(address)), rlp.encode(bytes.fromhex(node.node_id)), rlp_reward_per,
         rlp.encode(client.staking.cfg.external_id), rlp.encode(node_name), rlp.encode(client.staking.cfg.website), rlp.encode(client.staking.cfg.details)]))
    gas = get_the_dynamic_parameter_gas_fee(data) + 21000 + 6000 + 12000
    estimated_gas = node.eth.estimateGas({"from": address, "to": node.ppos.stakingAddress, "data": data})
    assert gas == estimated_gas
    estimated_edit_gas_fees = estimated_gas * node.eth.gasPrice
    result = client.ppos.editCandidate(address, node.node_id, client.staking.cfg.external_id, node_name, client.staking.cfg.website, client.staking.cfg.details,
    ...
    staking_address, pri_key = client.economic.account.generate_account(client.node.web3, client.economic.create_staking_limit + staking_gas_fees + estimated_edit_gas_fees)
    time.sleep(2)
    balance = node.eth.getBalance(staking_address)
    result = client.staking.create_staking(0, staking_address, staking_address, amount=client.economic.create_staking_limit)
    assert_code(result, 0)
    time.sleep(3)
...
def test_estimated_gas_005(client_new_node):
    ...
    result = client.staking.create_staking(0, staking_address, staking_address, amount=econnmic.create_staking_limit, reward_per=0)
    assert_code(result, 0)
    time.sleep(2)
    ...
    data = rlp.encode(
        [rlp.encode(int(1002)), rlp.encode(bytes.fromhex(node.node_id)), rlp.encode(0),
         rlp.encode(econnmic.add_staking_limit)])
    gas = get_the_dynamic_parameter_gas_fee(data) + 21000 + 6000 + 20000
    estimated_increase_gas = node.eth.estimateGas({"from": staking_address, "to": node.ppos.stakingAddress, "data": data})
    assert gas == estimated_increase_gas
...
def test_estimated_gas_006(client_new_node):
    ...
    data = rlp.encode([rlp.encode(int(1003)), rlp.encode(bytes.fromhex(node.node_id))])
    estimated_withdrew_gas = node.eth.estimateGas({"from": staking_address, "to": node.ppos.stakingAddress, "data": data})
...
def test_estimated_gas_007(client_new_node):
    ...
    result = client.staking.create_staking(0, staking_address, staking_address, amount=economic.create_staking_limit, reward_per=0)
    assert_code(result, 0)
...
def test_estimated_gas_008(client_new_node):
    """
       @describe: 锁仓预估gas，账户余额 = 交易金额 + gas * gasPrice, 可以预估成功，且锁仓成功
       @step:
    ...
    estimated_restricting_gas = node.eth.estimateGas({"to": client.ppos.restrictingAddress, "data": data, "from": address})
...
def test_estimated_gas_009(client_new_node):
    ...
    delegate_address, _ = economic.account.generate_account(node.web3, economic.delegate_limit + delegate_gas + gas_fees)
    result = client.delegate.delegate(0, delegate_address)
    assert_code(result, 0)
    balance = node.eth.getBalance(delegate_address)
    assert balance == gas_fees
    data = rlp.encode([rlp.encode(int(5000))])
    estimated_withdrew_gas = node.eth.estimateGas({"to": delegate_address, "data": data, "from": node.ppos.delegateRewardAddress})
